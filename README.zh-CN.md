...
- **小憩模式（Nap）** —— 纯黑（`#000`）全屏屏保，让所有背景像素完全熄灭。闲置数分钟后平滑淡入（可选 30 秒渐进变暗），也可通过会话头部的 **☾ Nap** 按钮手动开启。屏幕上有一个缓慢漂移的暗色时钟（避免时钟本身烧屏），实时显示智能体状态——*正在工作 / 等待你的输入 / 空闲*——任何鼠标或键盘输入都会唤醒屏幕。
...
| Gradual wind-down（渐进变暗） | 在自动小憩前的 30 秒内将屏幕渐变为黑色 |
